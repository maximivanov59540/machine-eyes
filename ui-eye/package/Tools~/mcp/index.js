...
const HELP = [
  ...
  "",
  "Поля ui_eye_shot: panel — имя из реестра (калибровочная — «проба»); states — какие состояния, пусто — все; " +
  ...
  "Сторож съёмщика — 180 с на всю пачку от входа в Play Mode (12 кадров «пробы» вместе с запуском Unity — 55 с): большой запрос делить.",
  "",
...
function problemWithShot(input) {
  if (typeof input.panel !== "string" || input.panel.trim() === "") {
    return "нужно имя панели в поле panel. Калибровочная — «проба»; реестр — ui_eye_help с panels: true.";
  }
...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        panel: {
          type: "string",
          description: "Имя панели из реестра ui-eye (методы с [UiEyePanel]). Калибровочная — «проба».",
        },
        states: {
          type: "array",
          items: { type: "string" },
